...
Signals are a core concept of Unix. Overall they offer a simple method of interprocess communication. When a program exits its return code, if the program crashed, will encode the signal. This offers a way for monitoring programs that crashed and why. The signals that result from a crash are listed at http://en.cppreference.com/w/c/program/SIG_types.
...
Core files contain a snapshot of information about the program at the moment before the program exited and at the time of the crash. Core files can be read by debugger programs like `gdb` and `lldb`. A common use of `gdb` and `lldb` is to generate a [backtrace](#backtrace) from core files.
...
The C/C++ language has a method called [backtrace and backtrace_symbols](http://man7.org/linux/man-pages/man3/backtrace_symbols.3.html) that can be used to print out the active stack frames. However, because C/C++ crashes might result from invalid use of memory, or out of memory conditions, it is dangerous to try to run more code, even these functions, after a crash. Therefore the convention to get backtraces for crashes is to enable [core file](#core file) generation and use `gdb` or `lldb` to generate a backtrace after the crash has happened.
...
The core pattern describes where the system should create core files. Therefore it usually consists of a path on the filesystem and one or more tokens that are populated to dynamically construct the filename.
